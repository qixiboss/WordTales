...
- **统一生词状态**：文章词卡、分类游戏、生词抄写、热力图和统计统一读取规范词条的 `isStarred`。
- **语境阅读与朗读**：28 个栏目提供原创短文、预生成 MP3、逐词高亮、段落解析和系统语音降级。
- **词汇练习**：支持词卡翻面、文章点词、全部词/生词游戏、桌面拼写和移动端手写。
- **学习进度**：文章接触、游戏分类、加星状态和记忆热力图保存在当前浏览器中。
- **响应式与打印**：文章、练习和进度面板均支持桌面端和移动端。
...
2. 浏览词卡并阅读短文；点击高亮词可查看释义并听发音，美音/英音偏好在页首切换。
3. 点击段尾“解析”查看翻译与语法说明，或选择倍速后朗读全文。
4. 使用“游戏”按熟悉程度分类；标为“不太认识”的词会加星。
5. 回到同一栏目点击“抄写”，集中练习已加星的词，并可从“今日学习进度”查看统计和热力图。
...
手动 smoke test 应覆盖：空 hash、`#library` 与旧 `#study`；栏目首次跳转和跨词集深链；统一星标、游戏、抄写、进度面板、美英音切换、文章朗读和解析；桌面和窄屏布局，以及 `file://` 打开。
